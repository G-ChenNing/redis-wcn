...
import com.github.wangchenning.redis.redis.KeyPrefix;
...
//@ConditionalOnBean(RedisPoolFactory.class)
@AutoConfigureAfter(RedisPoolFactory.class)
public class RedisService {
    ...
    public <T> List<T> getList(KeyPrefix prefix, String key, Class<T> clazz) {
        ...
        try {
            ...
            String realKey = prefix.getPrefix() + key;
            String str = jedis.get(realKey);
            String s = stringToBean(str, String.class);
            List<T> list = JSONArray.parseArray(s, clazz);
            return list;
        } finally {
            returnToPool(jedis);
        }
